...
def load_macro_data(
    ...
    # CPI file format can include multiple columns (World/Asia/Americas/Europe).
    # We explicitly map the `world` column into the pipeline's `CPI` macro series.
    date_col_candidates = [c for c in df_cpi.columns if str(c).strip().lower() == 'date']
    date_col_idx = df_cpi.columns.get_loc(date_col_candidates[0]) if date_col_candidates else 0

    world_col_candidates = [c for c in df_cpi.columns if str(c).strip().lower() == 'world']
    if world_col_candidates:
        world_col_name = world_col_candidates[0]
        world_col_idx = df_cpi.columns.get_loc(world_col_name)
    else:
        # Backwards-compatible fallback: assume the CPI series is in the 2nd column.
        world_col_idx = 1
        world_col_name = df_cpi.columns[world_col_idx]

    df_cpi_cleaned = clean_dataframe(df_cpi, date_col_idx, world_col_idx)

    # Standardize date column name for downstream merging.
    df_cpi_cleaned = df_cpi_cleaned.rename(columns={df_cpi_cleaned.columns[date_col_idx]: 'Date'})

    # Create CPI from the (cleaned) world series. Preserve the original World column
    # for exploratory analysis.
    world_col_name_cleaned = df_cpi_cleaned.columns[world_col_idx]
    df_cpi_cleaned['CPI'] = pd.to_numeric(df_cpi_cleaned[world_col_name_cleaned], errors='coerce')

    df_cpi_cleaned = df_cpi_cleaned.dropna(subset=['Date', 'CPI'])
